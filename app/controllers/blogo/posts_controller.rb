module Blogo
  ...
  class PostsController < Blogo::ApplicationController
    ...
    def search
     @posts_scope = Blogo::Post.published

      unless params[:post].nil?
        unless params[:post][:title].nil?
        	
		@posts_scope = 	Blogo::Post.published.search params[:post][:title]

		abort(@posts_scope.records.inspect)

          @posts_scope = @posts_scope.search(params[:post][:title]).records.results
        end
      end
      @meta = {}
      @meta[:title]     = "#{Blogo.config.site_title} - #{Blogo.config.site_subtitle}"
