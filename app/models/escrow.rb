class Escrow < ActiveRecord::Base
	...
	MONEYFDB = :money_fee_debited
	MONEYFCR = :money_fee_debited
	...
	def payment_status
		...
		if [3,4,5,6,8,10].include?(status.to_i) 
			return get_status
		end
		...
		if tn_role == "seller" || tn_role == "buyer" 
			...
			if fee_payer == amount_payer
				...
				if seller_accepted && buyer_accepted
					...
					return "buyer_and_seller_both_not_paid"	
				elsif (!seller_accepted && buyer_accepted)	
	...
	def refund_main_amount
		tncurrency = Currency.where(code: tn_currency).last
		shippingcurrency = Currency.where(code: shipping_currency).last

		if amount_payer == "seller"
			puts "---------------------11111111--------------------"
			#main amount
			if seller_accepted
				puts "-------------33333333-----------------------"
	...
	def refund_tnx_fee
		...
		if fee_payer == "seller"
			...
			if seller_accepted
				tn_account = seller.accounts.where(currency: tncurrency.id).last
				#tn amount
				unlock_fund(tn_account, tn_fee, Escrow::MONEYFCR.to_s+":#{id}")
			end
		end
		...
		if fee_payer == "buyer"
			...
			if buyer_accepted
				...
				tn_account = buyer.accounts.where(currency: tncurrency.id).last
				#tn amount
				unlock_fund(sh_account, tn_fee, Escrow::MONEYFCR.to_s+":#{id}")
			end
		end
	end
	...
	def sub_funds(account, amount, fee, reason, ref)
  	...
  	def lock_main_amount user
  		if amount_payer == user_role(user.email)
	  		tncurrency = Currency.where(code: tn_currency).last
	...
	def notify_users
		begin
			...
			if status.to_i == 4 #approved by admin
				...
				if fee_payer == "seller"
					tn_account = seller.accounts.where(currency: tncurrency.id).last
					#tn amount
				  	unlock_and_sub_funds tn_account, tn_fee, 0.0, Escrow::MONEYDB.to_s+":#{id}"
				end
				if amount_payer == "buyer"
					#main amount
					tn_account = buyer.accounts.where(currency: tncurrency.id).last
			...
			if status.to_i == 5 #declined by admin
				puts "----------000000000000-----------------"
			...
			#send signup request if user does not have account in the system. 
